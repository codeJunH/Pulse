...
export async function execute(
  ...
  const messages = currentScript
    ? [
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${QUERY_CONTEXT_EDIT.replace('%{database-schema}%', dbCollectionsSource).replace(
            '%{code}%',
            currentScript
          )}` : QUERY_CONTEXT_EDIT.replace('%{database-schema}%', dbCollectionsSource).replace(
            '%{code}%',
    ...
    : [
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${QUERY_CONTEXT_CREATE.replace('%{database-schema}%', dbCollectionsSource)}` : QUERY_CONTEXT_CREATE.replace('%{database-schema}%', dbCollectionsSource)
        },
        ...history
      ];
  ...
  await chatStreamXml({
    messages: [
      {
        role: 'system',
        content: aiRules ? `${aiRules}\n\n${QUERY_CONTEXT_EXPLAINER()}` : QUERY_CONTEXT_EXPLAINER()
      },
