    ...
    try {
      ...
      if (simulatedDocSearchResult || projectAnalysisResult) {
        ...
        if (nodeSuggestion) {
            ...
            aiResponse += `특정 노드를 생성하거나 연결하는 방법에 대한 정보를 찾지 못했습니다. 어떤 기능을 구현하려고 하시는지, 또는 어떤 종류의 노드를 찾으시는지 더 자세히 알려주시면 맞춤형 노드 제안을 드릴 수 있습니다.`;
        }

      } else {
        aiResponse = `죄송합니다. 현재 사용자님의 질문 \"${userPrompt}\"에 대한 Noodl 공식 문서나 프로젝트 관련 정보를 찾을 수 없습니다.`;
      }
