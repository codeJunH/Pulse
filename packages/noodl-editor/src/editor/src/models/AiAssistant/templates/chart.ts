  ...
  onMessage: async ({ node, chatHistory, chatStream, chatStreamXml }) => {
    ...
    const messages = currentScript
      ? [
          {
            role: 'system',
            content: aiRules ? `${aiRules}\n\n${CONTEXT_EDIT.replace('%{code}%', currentScript).replace('%{data}%', shortDataJson)}` : CONTEXT_EDIT.replace('%{code}%', currentScript).replace('%{data}%', shortDataJson)
          },
          history.at(-1)
        ]
      : [
          {
            role: 'system',
            content: aiRules ? `${aiRules}\n\n${CONTEXT.replace('%{data}%', shortDataJson)}` : CONTEXT.replace('%{data}%', shortDataJson)
          },
          ...history
        ];
