...
A["FUNCTION"]`;
  ...
  if (question.commands[0].args[0] === 'QUESTION') {
    ...
    const fullText = await chatStream({
      ...
      messages: [
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${FUNCTION_CODE_QUESTION().replace('%{code}%', currentScript)}` : FUNCTION_CODE_QUESTION().replace('%{code}%', currentScript)
        },
        {
          role: 'user',
          content: lastMessage
  ...
  const messages = currentScript
    ? [
        ...
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${FUNCTION_CODE_CONTEXT_EDIT.replace('%{code}%', currentScript)}` : FUNCTION_CODE_CONTEXT_EDIT.replace('%{code}%', currentScript)
        },
        history.at(-1)
      ]
    : [
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${FUNCTION_CODE_CONTEXT}` : FUNCTION_CODE_CONTEXT
        },
        ...history
      ];
