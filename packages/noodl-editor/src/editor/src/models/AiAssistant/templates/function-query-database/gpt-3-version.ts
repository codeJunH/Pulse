...
export async function generate(
  ...
  const messages = currentScript
    ? [
        ...
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${FUNCTION_QUERY_DATABASE_CONTEXT_EDIT_GPT3.replace('%{code}%', currentScript).replace(
            '%{database-schema}%',
            dbCollectionsSource
          )}` : FUNCTION_QUERY_DATABASE_CONTEXT_EDIT_GPT3.replace('%{code}%', currentScript).replace(
            '%{database-schema}%',
    ...
    : [
        {
          role: 'system',
          content: aiRules ? `${aiRules}\n\n${FUNCTION_QUERY_DATABASE_CONTEXT_GPT3.replace('%{database-schema}%', dbCollectionsSource)}` : FUNCTION_QUERY_DATABASE_CONTEXT_GPT3.replace('%{database-schema}%', dbCollectionsSource)
        },
        ...history
      ];
  ...
  const fullText = await chatStreamXml({
    messages: [
      {
        role: 'system',
        content: aiRules ? `${aiRules}\n\n${FUNCTION_QUERY_DATABASE_CONTEXT_EXPLAINER}` : FUNCTION_QUERY_DATABASE_CONTEXT_EXPLAINER
      },
