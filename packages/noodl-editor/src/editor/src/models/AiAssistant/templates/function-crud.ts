  ...
  onMessage: async ({ node, chatHistory, chatStreamXml }: IAiCopilotContext) => {
    ...
    const messages = currentScript
      ? [
          ...
          {
            role: 'system',
            content: aiRules ? `${aiRules}\n\n${FUNCTION_CRUD_CONTEXT_EDIT.replace('%{database-schema}%', dbCollectionsSource).replace('%{code}%', currentScript)}` : FUNCTION_CRUD_CONTEXT_EDIT.replace('%{database-schema}%', dbCollectionsSource).replace('%{code}%', currentScript)
          },
          history.at(-1)
        ]
      : [
          {
            role: 'system',
            content: aiRules ? `${aiRules}\n\n${FUNCTION_CRUD_CONTEXT.replace('%{database-schema}%', dbCollectionsSource)}` : FUNCTION_CRUD_CONTEXT.replace('%{database-schema}%', dbCollectionsSource)
          },
          ...history
        ];
